...
@role_required(['Admin'])
def review_veteran_form(request, form_id):
    """View to review a veteran certification form"""
    form = get_object_or_404(VeteranCertificationForm, id=form_id)
    ...
    if request.method == 'POST':
        ...
        if not decision:
            messages.error(request, 'Please select a decision (Pass/Fail)')
            return render(request, 'ums/review_veteran.html', {'form': form})
        
        try:
            if action == 'approve':
                form.status = 'approved'
                form.decision = 'Pass'  # Explicitly set to Pass when approving
                messages.success(request, 'Veteran certification request approved successfully.')
                ...
                if form.organizational_unit:
                    DocumentApproval.objects.create(
                        ...
                        approver=admin_user,
                        action="Approved",
                        comments=f"Veteran Certification Form approved by {admin_user.name}",
                        organizational_unit=form.organizational_unit,
                        is_org_level_approval=is_org_approver if 'is_org_approver' in locals() else False
                    )
                ...
                form.status = 'denied'
                form.decision = 'Fail'  # Always set to Fail when denied
                messages.success(request, 'Veteran certification request denied.')
                ...
                if form.organizational_unit:
                    DocumentApproval.objects.create(
                        ...
                        approver=admin_user,
                        action="Denied",
                        comments=f"Veteran Certification Form denied by {admin_user.name}",
                        organizational_unit=form.organizational_unit,
                        is_org_level_approval=is_org_approver if 'is_org_approver' in locals() else False
                    )
    ...
    return render(request, 'ums/review_veteran.html', {
        'form': form,
        'org_context': org_context
    })
